...
function Men({ cart, setCart }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`http://localhost:https://draipe-backend.onrender.com//api/products/men?page=${page}&limit=50`)
      .then(res => res.json())
      .then(data => {
          ...
          {menProducts.map(product => (
            ...
            >
              ...
              <div className="product-info" style={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                <div className="product-name" style={{ fontWeight: 600, fontSize: 16, marginBottom: 6, minHeight: 40 }}>
                  {product.name}
                </div>
                <div className="product-price" style={{ color: '#333', marginBottom: 12 }}>
                  ₹{product.price}
                </div>
                <button
